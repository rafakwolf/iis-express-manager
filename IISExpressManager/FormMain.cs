    ...
    public partial class FormMain : Form
    {
        ...
        private void ListViewInsertItems()
        {
            foreach (IISSites iisSite in _iisSites)
            {
                ...
                listViewApps.Items[listViewApps.Items.Count - 1].UseItemStyleForSubItems = false;
        ...
        {
            int selected = -1;
            foreach (int index in listViewApps.SelectedIndices)
            {
                selected = index;
            }
            return selected;
        ...
        private void listView1_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                cntxIISSite.Show(this.listViewApps, e.Location);
            }
            ...
            if (selected >= 0)
            {
                textBoxAppPath.Text = _iisSites[selected].Folder;
            }
        ...
        private void deleteSiteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    @"Primeiro finalize antes de excluir.",
                                "Error", MessageBoxButtons.OK,
                                MessageBoxIcon.Error);
                return;
            }
            ...
            if (selected >= 0)
            {
                if (MessageBox.Show(@"Deseja realmente excluir este site ?", @"Confirmação", MessageBoxButtons.YesNo) ==
                    DialogResult.Yes)
                {
                    IISProcessManager.DeleteSite(_iisSites[selected].SiteName);
